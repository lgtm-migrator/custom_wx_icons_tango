...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  autopep8 --in-place --select "$errors" -a --recursive wx_icons_tango/
  autopep8 --in-place --select "$belligerent" -a -a -a -a -a --recursive wx_icons_tango/


  echo "Running flake8"

    >&2 flake8 wx_icons_tango/
